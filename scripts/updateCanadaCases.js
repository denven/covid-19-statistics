	...
	if (res.status === 200) {
		...
		const getOneMonthReports = (reports, month) => {
			...
			reports.forEach((item) => {
				if (item.date.includes(month)) tmp.push(item);
			});
			return tmp;
		};
		...
		tmpCases = [
			...
			...getOneMonthReports(tmp, "August"),
			...getOneMonthReports(tmp, "September"),
			...getOneMonthReports(tmp, "October"),
			...getOneMonthReports(tmp, "November"),
			...getOneMonthReports(tmp, "December"),
		];
...
const getTodaysCases = (elements, $, allDaysCases) => {
	...
	try {
		...
		elements.each((index, item) => {
			...
			if (index === 1) {
				// wikipedia changed table format Mar 19, 2020
				provAbbrs = $(item)
					.text()
					.trim()
					.replace(/\n{1,5}/g, ",")
